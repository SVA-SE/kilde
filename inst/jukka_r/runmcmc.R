...
for(j in 1:nat[1]){ 
  ASPpar0[i,j] <- rgamma(1,sourcesASP[i,j]+FULL*sum(S[,i]*IASP[,j]>0)+1/nat[1],1)
}
for(j in 1:nat[1]){
  qASP[mc,i,j] <-  (ASPpar0[i,j]+h) / (sum(ASPpar0[i,])+h*nat[1])
}
for(j in 1:nat[2]){ 
  GLNpar0[i,j] <- rgamma(1,sourcesGLN[i,j]+FULL*sum(S[,i]*IGLN[,j]>0)+1/nat[2],1)
}
for(j in 1:nat[2]){
  qGLN[mc,i,j] <-  (GLNpar0[i,j]+h) / (sum(GLNpar0[i,])+h*nat[2])
}
for(j in 1:nat[3]){ 
  GLTpar0[i,j] <- rgamma(1,sourcesGLT[i,j]+FULL*sum(S[,i]*IGLT[,j]>0)+1/nat[3],1)
}
for(j in 1:nat[3]){
  qGLT[mc,i,j] <-  (GLTpar0[i,j]+h) / (sum(GLTpar0[i,])+h*nat[3])
}
for(j in 1:nat[4]){ 
  GLYpar0[i,j] <- rgamma(1,sourcesGLY[i,j]+FULL*sum(S[,i]*IGLY[,j]>0)+1/nat[4],1)
}
for(j in 1:nat[4]){
  qGLY[mc,i,j] <-  (GLYpar0[i,j]+h) / (sum(GLYpar0[i,])+h*nat[4])
}
for(j in 1:nat[5]){ 
  PGMpar0[i,j] <- rgamma(1,sourcesPGM[i,j]+FULL*sum(S[,i]*IPGM[,j]>0)+1/nat[5],1)
}
for(j in 1:nat[5]){
  qPGM[mc,i,j] <-  (PGMpar0[i,j]+h) / (sum(PGMpar0[i,])+h*nat[5])
}
for(j in 1:nat[6]){ 
  TKTpar0[i,j] <- rgamma(1,sourcesTKT[i,j]+FULL*sum(S[,i]*ITKT[,j]>0)+1/nat[6],1)
}
for(j in 1:nat[6]){
  qTKT[mc,i,j] <-  (TKTpar0[i,j]+h) / (sum(TKTpar0[i,])+h*nat[6])
}
for(j in 1:nat[7]){ 
  UNCpar0[i,j] <- rgamma(1,sourcesUNC[i,j]+FULL*sum(S[,i]*IUNC[,j]>0)+1/nat[7],1)
}
for(j in 1:nat[7]){
