...
for(i in 1:ns){plot(phi[burnin:MCMC,i],xlab="",ylab=sourcenames[i],pch=16,cex=0.5,cex.lab=1.7)}
dev.new()
...
par(mfrow=c(2,4))
errorASP <- matrix(NA,ns-1,nat[1]);errorGLN <- matrix(NA,ns-1,nat[2]);
errorGLT <- matrix(NA,ns-1,nat[3]);errorGLY <- matrix(NA,ns-1,nat[4]);
errorPGM <- matrix(NA,ns-1,nat[5]);errorTKT <- matrix(NA,ns-1,nat[6]);
...
for(i in 1:ns){for(j in 1:nat[1]){
...
for(i in 1:ns){for(j in 1:nat[2]){
...
for(i in 1:ns){for(j in 1:nat[3]){
...
for(i in 1:ns){for(j in 1:nat[4]){
...
for(i in 1:ns){for(j in 1:nat[5]){
...
for(i in 1:ns){for(j in 1:nat[6]){
...
for(i in 1:ns){for(j in 1:nat[7]){
